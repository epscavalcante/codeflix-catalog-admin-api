...
import { GenreCategoryModel, GenreModel } from '@core/genre/infra/database/sequelize/models/genre.model';
...
describe('CreateGenreUseCase Unit Tests', () => {
    ...
    const database = setupDatabase({ models: [GenreModel, CategoryModel, GenreCategoryModel] });
    ...
    test('Deve criar um gênero', async () => {
        ...
        const genreModel = await genreRepository.findById(new GenreId(output.id));
        const categoriesRelated = await categoryRepository.findByIds(categories.map(category => category.categoryId));;
        ...
        expect(output).toStrictEqual({
            ...
            categoriesId: categoriesRelated.map(category => category.categoryId.value)
        })
    });
});
