...
class StubSearchableMemoryRepository extends SearchableMemoryRepository<EntityStub, Uuid> {
    ...
    protected async applyFilter(
        ...
        return items.filter(item => {
            return (item.name.toLowerCase().includes(filter.toLowerCase()) || item.price.toString() === filter)
        })
    }
}
...
describe('Searchable Memomy UnitTests', () => {
    ...
    describe('ApplyFilters method', () => {
        ...
        test('Should filter items', async () => {
            ...
            itemsFiltered = await repository['applyFilter'](items, 'hello world')
